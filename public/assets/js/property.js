(() => {
  ...
  function visibilityControl() {
    if (!data.permissions?.canManageVisibility) return '';
    const isPrivate = data.listing.visibility === 'private';
    return `<div class="property-panel property-visibility"><span>Visibilité</span><div role="group" aria-label="Visibilité de l’annonce"><label><input type="radio" name="visibility" value="shared" data-visibility ${isPrivate ? '' : 'checked'}> Publique</label><label><input type="radio" name="visibility" value="private" data-visibility ${isPrivate ? 'checked' : ''}> Privée</label></div></div>`;
  }
  async function updateVisibility(event) {
    const controls = [...panel().querySelectorAll('[data-visibility]')];
    controls.forEach(control => { control.disabled = true; });
    try {
      await saveFields({ visibility: event.target.value });
      renderListing();
      toast(event.target.value === 'private' ? 'Annonce privée.' : 'Annonce publique.');
    } catch (error) {
      renderListing();
      toast(error.message);
    }
