(() => {
  ...
  async function renderPrices() {
    panel().innerHTML = state('prix', 'Recherche des ventes comparables', 'Interrogation des mutations DVF les plus récentes…', '<div class="analysis-spinner" aria-hidden="true"></div>');
    try {
      const prices = await request(new URL(`prices.php?id=${encodeURIComponent(id)}`, API_ROOT));
