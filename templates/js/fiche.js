(() => {
  ...
  function negotiationContext(negotiation) {
    ...
    const requiredNumbers = [scale.valeur_prudente_min_eur, scale.valeur_prudente_max_eur, scale.valeur_prudente_centrale_eur, scale.offre_recommandee_min_eur, scale.offre_recommandee_max_eur, scale.prix_demande_eur, scale.cout_total_eur, coherence.prix_demande_m2_eur, commune.prix_m2_eur, commune.ecart_pct, neighborhood.prix_m2_eur, neighborhood.ecart_pct];
    const available = requiredNumbers.every(Number.isFinite) && references.length > 0;
    if (!available) return { negotiationAvailable: false };
    const values = requiredNumbers.slice(0, 7);
    ...
    const range = (from, to) => `${euro(from)} – ${euro(to)}`;
    const marketRows = references.map(item => ({ source: item.source || '—', propertyType: item.type_bien || '—', pricePerSqm: euro(item.prix_m2_eur), range: Number.isFinite(item.fourchette_min_eur_m2) && Number.isFinite(item.fourchette_max_eur_m2) ? range(item.fourchette_min_eur_m2, item.fourchette_max_eur_m2) : '—', precision: item.precision || '', referenceDate: item.date || '—' }));
    marketRows.push({ source: 'Bien analysé', propertyType: analyzed.type_bien || '—', pricePerSqm: euro(analyzed.prix_m2_eur), range: analyzed.fourchette_ou_commentaire || '—', precision: '', referenceDate: analyzed.date || '—', analyzed: true });
    return { negotiationAvailable: true, prudentRange: range(scale.valeur_prudente_min_eur, scale.valeur_prudente_max_eur), prudentEstimate: scale.valeur_prudente_estimee, offerRange: range(scale.offre_recommandee_min_eur, scale.offre_recommandee_max_eur), askingPriceNegotiation: euro(scale.prix_demande_eur), totalCostNegotiation: euro(scale.cout_total_eur), prudentStyle: rangeStyle(scale.valeur_prudente_min_eur, scale.valeur_prudente_max_eur), offerStyle: rangeStyle(scale.offre_recommandee_min_eur, scale.offre_recommandee_max_eur), askingStyle: `left:${position(scale.prix_demande_eur)}`, totalStyle: `left:${position(scale.cout_total_eur)}`, negotiationArgument: scale.argumentaire || '—', askingPricePerSqm: euro(coherence.prix_demande_m2_eur), surfaceReference: coherence.surface_reference_m2 ? `${coherence.surface_reference_m2} m² habitables` : 'surface non renseignée', communeGap: signedPercent(commune.ecart_pct), communeReference: `${euro(commune.prix_m2_eur)}/m² · ${commune.source || 'source non renseignée'} · ${commune.date || 'date non renseignée'}`, neighborhoodGap: signedPercent(neighborhood.ecart_pct), neighborhoodReference: `${euro(neighborhood.prix_m2_eur)}/m² · ${neighborhood.source || 'source non renseignée'} · ${neighborhood.date || 'date non renseignée'}`, marketRows, marketLimits: coherence.perimetre_et_limites || '—' };
  }

  function patrimonialContext(a) {
    const property = a.bien || {}, address = property.adresse || {}, features = property.caracteristiques || {}, acquisition = a.acquisition || {}, quality = a.qualite_patrimoniale || {}, access = a.accessibilite_depuis_paris || {}, route = access.itineraire_recommande || {}, arrival = route.gare_arrivee || {}, frequencies = access.frequences || {}, lastMile = access.dernier_kilometre || {}, roundTrip = access.aller_retour_journee || {}, decision = a.decision || {};
    const negotiation = a.negociation || {};
    const decisionClass = decision.verdict === 'go' ? 'go' : decision.verdict === 'no_go' ? 'nogo' : 'cond';
    const decisionColor = decisionClass === 'go' ? '#145a2e' : decisionClass === 'nogo' ? '#831515' : '#7a4108';
    const scenario = (a.financement?.scenarios || []).find(item => item.code === a.financement?.scenario_retenu) || {};
    ...
    return { ...negotiationContext(negotiation), source: property.source || '—', reference: property.reference || '—', city: address.ville || '—', postalCode: address.code_postal || '—', propertyType: label(property.type), surface: features.surface_habitable_m2 ?? '—', landSurface: features.surface_terrain_m2 ?? '—', dpe: features.dpe || '—', analysisDate: date(a.meta?.date_analyse), promptVersion: a.meta?.version_prompt || '—', schemaVersion: a.meta?.version_schema || '—', score: decision.score_global ?? '—', scoreOffset: Math.max(0, 201.1 * (1 - Number(decision.score_global || 0) / 100)).toFixed(1), decision: label(decision.verdict), decisionClass, decisionColor, askingPrice: euro(acquisition.prix_affiche_eur), askingPriceRaw: acquisition.prix_affiche_eur ?? 0, notaryFeesRaw: acquisition.frais_notaire_eur ?? 0, worksRaw: Number(acquisition.travaux_immediats_eur || 0) + Number(acquisition.travaux_patrimoniaux_eur || 0), annexFeesRaw: acquisition.frais_annexes_eur ?? 0, totalCost: euro(acquisition.cout_total_projet_eur), scenarioName: label(decision.scenario_recommande), contribution: euro(scenario.apport_eur), netCost: euro(scenario.cout_net_apres_optimisation_eur), summary: decision.synthese_argumentee || decision.synthese || 'Résumé indisponible.', strength: decision.point_fort || '—', watchout: decision.point_vigilance || '—', conditions: (decision.conditions_go || []).map((text, index) => ({ number: String(index + 1).padStart(2, '0'), text })), departureStation: access.origine?.gare_depart || 'Paris', arrivalStation: arrival.nom || '—', trainTypes: (route.trains || []).join(' · ') || '—', trainMinimum: duration(route.duree_train_min), totalMinimum: duration(route.duree_totale_min), totalTypical: duration(route.duree_totale_typique), directLabel: route.direct ? 'Direct' : `${route.nombre_correspondances ?? '—'} correspondance(s)`, robustness: label(route.robustesse), weekdayFrequency: frequencies.jour_ouvre?.liaisons_exploitables ?? '—', saturdayFrequency: frequencies.samedi?.liaisons_exploitables ?? '—', sundayFrequency: frequencies.dimanche?.liaisons_exploitables ?? '—', fridayComment: frequencies.vendredi_soir_commentaire || '—', sundayComment: frequencies.dimanche_soir_retour_commentaire || '—', travelComment: access.evaluation?.commentaire_argumente || frequencies.commentaire || '—', lastMileMode: label(lastMile.mode_recommande), lastMileDistance: typeof lastMile.distance_km === 'number' ? `${lastMile.distance_km.toLocaleString('fr-FR')} km` : '—', lastMileDuration: duration(lastMile.duree_min), carRequired: lastMile.voiture_indispensable ? 'Oui' : 'Non', roundTrip: roundTrip.possible ? 'Possible' : 'Non recommandé', roundTripComment: roundTrip.commentaire || '—', accessScore: access.evaluation?.score ?? '—', accessVerdict: label(access.evaluation?.verdict), alternatives: (access.alternatives || []).map(item => ({ ...item, duration: duration(item.duree_totale_min), directLabel: item.direct ? 'Direct' : `${item.nombre_correspondances ?? '—'} correspondance(s)` })), quality: [{ label: 'Emplacement', value: label(quality.qualite_emplacement) }, { label: 'Rareté', value: label(quality.rarete) }, { label: 'Liquidité', value: label(quality.liquidite_revente) }, { label: 'Bâti', value: label(quality.qualite_bati) }, { label: 'Valorisation', value: label(quality.potentiel_valorisation) }, { label: 'Horizon recommandé', value: quality.horizon_recommande_ans ? `${quality.horizon_recommande_ans} ans` : '—' }], scoringAxes, levers: Object.entries(a.leviers_optimisation || {}).filter(([, value]) => value && typeof value === 'object' && 'faisabilite' in value).map(([key, value]) => ({ name: label(key), feasibility: label(value.faisabilite), applicable: value.applicable ? 'Applicable' : 'Non applicable', central: euro(value.resultat_net_central_eur ?? value.revenu_net_annuel_central_eur), degraded: euro(value.resultat_net_degrade_eur ?? value.revenu_net_annuel_degrade_eur), conditions: (value.conditions || []).join(' · ') || '—' })), scenarios: (a.financement?.scenarios || []).map(item => ({ name: label(item.code), applicable: item.applicable ? 'Oui' : 'Non', deposit: `${item.apport_pct ?? '—'} % · ${euro(item.apport_eur)}`, loan: euro(item.capital_emprunte_eur), payment: euro(item.mensualite_credit_eur), effort: euro(item.effort_mensuel_eur), netCost: euro(item.cout_net_apres_optimisation_eur) })), risks: (a.risques || []).map(item => ({ ...item, criticality: label(item.criticite), criticalityClass: item.criticite === 'critique' ? 'rc' : item.criticite === 'elevee' || item.criticite === 'eleve' ? 're' : 'rm', impactClass: item.criticite === 'critique' ? 'tr' : 'tw' })), beforeOffer: a.diligences_avant_offre || [], beforeContract: a.diligences_avant_compromis || [], sources: (a.sources || []).map(item => ({ label: item.label || item })) };
  }
  ...
  async function render() {
    ...
    try {
      ...
      app.className = ''; app.innerHTML = renderTemplate(document.getElementById('fiche-template').innerHTML, context); setupTabs(app); setupFinanceEditor(app); setupGallery(payload.listing); addRecalculateButton();
